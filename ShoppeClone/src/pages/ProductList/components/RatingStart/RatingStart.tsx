  ...
  return (
    <ul className='my-3'>
      {Array(5)
        .fill(0)
        .map((_, index) => (
          <li className='py-1 pl-4 cursor-pointer' key={index}>
            <div className='flex items-center' onClick={() => handleFilterStart(5 - index)} aria-hidden='true'>
              {Array(5)
                .fill(0)
                .map((_, indexStart) => {
                  if (indexStart < 5 - index) {
                    return (
                      <svg key={indexStart} viewBox='0 0 9.5 8' className='w-4 h-4 mr-1'>
                        <defs>
